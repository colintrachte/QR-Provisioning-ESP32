 ...
 * ─────────────────────
 *  - DNS response IP is now derived from WIFI_MANAGER_AP_IP at compile time
 *    via sscanf so it stays in sync if the macro changes.
 *  - CREDS_RECEIVED event now stores the SSID in s_sta_ssid so the
 *    subsequent STA_CONNECTING state in prov_ui has the right name to show.
 *  - NVS init is idempotent: if the caller already initialised NVS the
 *    ESP_ERR_NVS_INVALID_STATE return is silently accepted.
 */
                             ...
                             const char  *mime_type)
{
    ...
    if (!f) {
        ESP_LOGE(TAG, "serve_file: OPEN FAILED: %s  (run pio run -t uploadfs)", path);
    ...
    if (!buf) {
        ESP_LOGE(TAG, "serve_file: malloc failed");
        fclose(f);
    ...
    while (send_ok && (n = fread(buf, 1, FS_CHUNK_SIZE, f)) > 0) {
        if (httpd_resp_send_chunk(req, buf, (ssize_t)n) != ESP_OK) {
            ESP_LOGE(TAG, "serve_file: send chunk failed for %s", path);
            send_ok = false;
        } else {
            total_sent += n;
        }
    ...
    ESP_LOGI(TAG, "serve_file: done %s  sent=%u/%ld", path,
             (unsigned)total_sent, file_size);
...
/* ── DNS IP octets derived from WIFI_MANAGER_AP_IP macro ───────────────────
 * Parsing at compile time would require constexpr or a complex macro;
 * instead we parse once at startup in dns_task so any change to the macro
 * is automatically reflected without touching dns_task's byte literals.
 */
static uint8_t s_gw_ip[4] = {192, 168, 4, 1}; /* fallback default */

static void parse_ap_ip(void)
{
    unsigned a, b, c, d;
    if (sscanf(WIFI_MANAGER_AP_IP, "%u.%u.%u.%u", &a, &b, &c, &d) == 4) {
        s_gw_ip[0] = (uint8_t)a;
        s_gw_ip[1] = (uint8_t)b;
        s_gw_ip[2] = (uint8_t)c;
        s_gw_ip[3] = (uint8_t)d;
        ESP_LOGI(TAG, "DNS redirect IP: %u.%u.%u.%u", a, b, c, d);
    } else {
        ESP_LOGW(TAG, "Could not parse WIFI_MANAGER_AP_IP='%s', using default",
                 WIFI_MANAGER_AP_IP);
    }
}
...
static void scan_task(void *arg)
{
    while (1) {
        ...
        ESP_LOGI(TAG, "scan_task: starting WiFi scan");
        ...
        if (err != ESP_OK) {
            ESP_LOGW(TAG, "scan_task: scan_start failed: %s — retrying in 3 s",
                     esp_err_to_name(err));
...
static void dns_task(void *arg)
{
    ...
    ESP_LOGI(TAG, "dns_task: listening on :%d -> %u.%u.%u.%u",
    ...
    while (1) {
        ...
        uint8_t resp[512];
        memcpy(resp, buf, len);
        /* Echo transaction ID from bytes 0-1 (already copied).
         * Set QR bit and RA bit; RCODE=0 (no error). */
        ...
        resp[rlen++] = 0xC0; resp[rlen++] = 0x0C; /* name ptr to offset 12 */
...
static esp_err_t handle_save(httpd_req_t *req)
{
    ...
    ESP_LOGI(TAG, "handle_save: saving SSID='%s'", ssid);

    /* Store SSID so the STA_CONNECTING state in prov_ui can show the right name. */
...
static void fs_mount(void)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "LittleFS mount failed: %s  (check partitions.csv)",
                 esp_err_to_name(err));
    } else {
...
static void event_task(void *arg)
{
    ...
    while (xQueueReceive(s_event_queue, &ev, portMAX_DELAY) == pdTRUE) {
        ...
        if (ev.state == WIFI_MANAGER_STATE_STA_CONNECTING) {
            /* Populate s_sta_ssid so prov_ui_on_state_change can read it. */
            if (ev.ssid[0]) {
...
esp_err_t wifi_manager_start(const wifi_manager_config_t *config)
{
    ...
    /* NVS init — idempotent: ESP_ERR_NVS_INVALID_STATE means it was already
     * initialised by the caller, which is fine. */
