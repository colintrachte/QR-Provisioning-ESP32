 ...
 * Generated by the linker from EMBED_FILES in CMakeLists.txt.
 * Naming rule: path separators and dots → underscores, prefixed with
 * _binary_, suffixed with _start / _end / _length.
 * "../data/settings.js.gz" → _binary_settings_js_gz_start, etc.
 */
...
/* ── Helper macro ──────────────────────────────────────────────────────────*/
/* Serves an embedded (flash-mapped) buffer with correct headers.
 * Always sets gzip encoding when the symbol came from a .gz file.
 * Cache-Control is long for assets that don't change without a firmware flash. */
...
/* NOTE: POST /api/settings is exclusively owned by settings_server.c.
 * The old handle_api_settings_post() that wrote raw JSON keys directly to NVS
 * (bypassing validation and the settings_mgr schema) has been removed.
 * settings_server_register() registers the correct handler at startup. */
...
{
    SERVE_EMBEDDED(req, "text/html", index_html_gz_start, index_html_gz_end, true, false);
    return ESP_OK;
}
...
{
    SERVE_EMBEDDED(req, "application/javascript", index_js_gz_start, index_js_gz_end, true, true);
    return ESP_OK;
}
static esp_err_t handle_settings(httpd_req_t *req)
{
    SERVE_EMBEDDED(req, "text/html", settings_html_gz_start, settings_html_gz_end, true, false);
    return ESP_OK;
}
static esp_err_t handle_settings_css(httpd_req_t *req)
{
    SERVE_EMBEDDED(req, "text/css", settings_css_gz_start, settings_css_gz_end, true, true);
    return ESP_OK;
}
static esp_err_t handle_settings_js(httpd_req_t *req)
{
    SERVE_EMBEDDED(req, "application/javascript", settings_js_gz_start, settings_js_gz_end, true, true);
    return ESP_OK;
}
static esp_err_t handle_favicon(httpd_req_t *req)
{
    SERVE_EMBEDDED(req, "image/svg+xml", favicon_svg_gz_start, favicon_svg_gz_end, true, true);
    return ESP_OK;
}
