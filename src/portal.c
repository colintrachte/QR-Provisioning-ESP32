...
/* ── Embedded portal assets (generated by PIO from data folder) ───────────── */
...
{
    return serve_embedded(req, "image/svg+xml",
...
static esp_err_t handle_base_css(httpd_req_t *req)
{
    return serve_embedded(req, "text/css",
                          base_css_gz_start, base_css_gz_end, true, true);
}
...
static esp_err_t handle_connect(httpd_req_t *req)
{
    ...
    body[n] = '\0';
    ESP_LOGI(TAG, "POST /api/connect raw body: '%s'", body);  // <-- add

    extract_field(body, "ssid",     s_ssid, sizeof(s_ssid));
    extract_field(body, "pass", s_pass, sizeof(s_pass));

    ESP_LOGI(TAG, "Decoded SSID='%s' PASS='%s'", s_ssid, s_pass);  // <-- add
...
void portal_start(const char *ap_ssid)
{
    ...
    /* Only scan on first call — radio is stable then.
     * On retries the AP is already up with a client connected;
     * re-scanning would hop channels and likely drop them. */
    if (!s_scan_ready) {
        run_scan_once();
    }
...
void portal_get_credentials(char *out_ssid, char *out_pass)
{
    ...
    ESP_LOGI(TAG, "portal_get_credentials: SSID='%s' PASS='%s'", out_ssid, out_pass);  // <-- add
}
