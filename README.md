...
### Stability & Polish (Next)

- [ ] **Unit tests** — ESP-IDF (https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/system/unit-tests.html). Targets: `qr_gen`, `url_decode`, NVS round-trip.
- [ ] **CI build** — [GitHub Actions](https://github.com/features/actions) with ESP-IDF Docker. Build ESP32-S3 on every push; fail on warnings.
- [ ] **JS error reporting** — `window.onerror` → POST to firmware endpoint so JS errors appear in serial log.
- [ ] **Favicon** — Serve real icon from LittleFS instead of 204.
- [ ] **Portal timeout** — Maybe not auto-close, but go into some sort of hibernation after N minutes of inactivity.

### Features

- [ ] **Dynamic user settings** — JSON in LittleFS for runtime config (calibration, UI template, LoRa region). Survives OTA.
- [ ] **UI template switching** — Store template name in settings; serve matching HTML/CSS/JS.
- [ ] **Peripheral config system** — Hardware profile from settings at boot. `i_sensors` scans I2C; extend to auto-configure.
- [ ] **Portal improvements** — Info page with build info + reset button, per-AP credential history, static IP option.

### Communications

- [ ] **WebSocket vs UDP evaluation** — Prototype UDP, compare latency + jitter under load.
- [ ] **BLE provisioning** — Alternative to SoftAP for phones that prefer BLE pairing.
- [ ] **LoRa integration** — Join [Meshtastic](https://meshtastic.org/) network or custom point-to-point. Enforce regional duty-cycle limits.
- [ ] **Home Assistant / MQTT** — Publish sensors, accept commands. Store broker in settings.
- [ ] **Device-to-device control** — One ESP32 as controller, another as actuator over WiFi/LoRa.

### Hardware

- [ ] **Board abstraction layer** — GPIO tables per board. Candidates:
  - [Heltec WiFi LoRa 32 V3](https://heltec.org/project/wifi-lora-32-v3/) (current)
  - [TTGO LoRa32 V1](https://github.com/LilyGO/TTGO-LORA32) (ESP32, AXP192, NEO-6M GPS)
  - [Heltec HTIT-Tracker](https://heltec.org/project/htit-tracker/) (ESP32-S3)
- [ ] **TTGO V1 peripherals** — AXP192 PMIC (battery, charge state), NEO-6M GPS, onboard temp.
- [ ] **SPI display support** — ST7735 / ILI9341 via u8g2.
- [ ] **E-paper display** — Low-power / solar use cases.
